...
<body class="bg-light" data-user-id="<?php echo $_SESSION['user_id']; ?>"
  data-user-name="<?php echo htmlspecialchars($admin_name); ?>" data-user-role="<?php echo $role; ?>"
  ...
  <div class="container py-2">
    <ul class="nav nav-tabs mb-3">
      <li class="nav-item">
        <a class="nav-link <?php if ($tab === 'reports')
          echo 'active'; ?>" href="admin_reports.php">Báo cáo</a>
      </li>
      <?php if ($role === 'admin' || $role === 'quanly' || $role === 'nhomtruong'): ?>
        <li class="nav-item">
          <a class="nav-link <?php if ($tab === 'users')
            echo 'active'; ?>" href="admin_reports.php?tab=users">Duyệt người
            dùng mới</a>
        </li>
      <?php endif; ?>
      <?php if ($role === 'admin' || $role === 'quanly' || $role === 'nhomtruong'): ?>
        <li class="nav-item">
          <a class="nav-link <?php if ($tab === 'roles')
            echo 'active'; ?>" href="admin_reports.php?tab=roles">Người
            dùng</a>
        </li>
      <?php endif; ?>
    </ul>
                  ...
                  while ($u = $pending_users->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td class="text-center">
                        ...
                        ?>
                        <?php if ($can_approve): ?>
                          <a href="admin_reports.php?tab=users&approve_user=<?php echo $u['id']; ?>"
                            class="btn btn-success btn-sm"><i class="fa-solid fa-check"></i> Duyệt</a>
                          <a href="admin_reports.php?tab=users&delete_user=<?php echo $u['id']; ?>"
                            class="btn btn-danger btn-sm" onclick="return confirm('Xóa người dùng này?');"><i
                      ...
                      <td>
                        <?php if ($role === 'admin'): ?>
                          <form method="post" style="display:inline-block;min-width:120px;">
                            ...
                            <select name="new_department_id" class="form-select form-select-sm d-inline w-auto"
                              style="min-width:100px;display:inline-block;">
                              <?php foreach ($departments_arr as $dept_id => $dept_name): ?>
                                <option value="<?php echo $dept_id; ?>" <?php if ($u['department_id'] == $dept_id)
                                     echo 'selected'; ?>><?php echo htmlspecialchars($dept_name); ?></option>
                              <?php endforeach; ?>
                            </select>
                ...
                <div class="col-md-3">
                  ...
                  <select name="user_id" class="form-select">
                    ...
                    while ($u = $users->fetch_assoc()): ?>
                      <option value="<?php echo $u['id']; ?>" <?php if (!empty($_GET['user_id']) && $_GET['user_id'] == $u['id'])
                           echo 'selected'; ?>><?php echo htmlspecialchars($u['name']); ?></option>
                    <?php endwhile; ?>
                  </select>
                </div>
                <div class="col-md-3">
                ...
                <tbody>
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td class="cell-content-limit" title="<?php echo htmlspecialchars($row['content']); ?>">
                        <?php echo htmlspecialchars(mb_strimwidth($row['content'], 0, 100, '...')); ?></td>
                      <td><?php echo $row['created_at']; ?></td>
                      <td class="text-center">
                        <div class="d-flex align-items-center justify-content-center gap-2">
                          <a href="view_report.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-info" title="Xem"><i
                              class="fa-solid fa-eye"></i></a>
                          <?php if ($role === 'admin' || $row['user_id'] == $_SESSION['user_id']): ?>
                            <a href="edit_report.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-primary"
                              title="Sửa"><i class="fa-solid fa-pen-to-square"></i></a>
                            <button onclick="confirmDelete(<?php echo $row['id']; ?>)" class="btn btn-sm btn-danger"
